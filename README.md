...
- Claim **не** выдаётся автоматически — клиент должен явно запросить его через `/voicer claim`
- При отключении клиента с claim — claim сбрасывается без передачи другому экземпляру
- Плагин автоматически подключается к Voicer при старте, но для получения транскрипций нужен явный `/voicer claim`

### Команды

Команды выполняются напрямую плагином без обращения к LLM:

| Команда | Описание |
|---------|----------|
| `/voicer claim` | Перехватить голосовой ввод на этот экземпляр |
| `/voicer release` | Отпустить голосовой ввод |
| `/voicer status` | Показать статус подключения и claim |
| `/voicer new` | Создать новую сессию |
| `/voicer` | То же что `/voicer status` |
...
| `voicer_new_session` | Создаёт новую сессию (по голосовой команде «новая сессия») |
