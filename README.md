...
- **Два режима работы:**
  - **WebSocket** (F6) — транслирует распознанный текст подключённым клиентам
  - **Вставка в курсор** (F7) — вставляет текст в активное приложение через Ctrl+V / Cmd+V
- **Захват выделенного текста** — в режиме WebSocket выделенный текст автоматически добавляется к надиктованному перед отправкой (отключаемо)
...
2. Удерживайте **F6** для записи (результат отправляется по WebSocket). Если перед нажатием был выделен текст — он будет добавлен к результату
3. Удерживайте **F7** для записи (результат вставляется в активное окно)
4. Правый клик по иконке в трее — настройки и выход
...
| WebSocket hotkey | F6 | Горячая клавиша (режим WS) |
| Insert hotkey | F7 | Горячая клавиша (режим вставки) |
| WS Port | 5050 | Порт WebSocket-сервера |
| Show popup | Вкл | Всплывающее уведомление |
| Include selected text | Вкл | Добавлять выделенный текст к надиктованному в режиме WS |
...
4. Запустите Voicer
5. Нажмите **F6**, произнесите команду — текст появится как промпт в OpenCode
